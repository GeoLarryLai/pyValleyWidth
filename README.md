...
Python tools to measure **river valley width** from a digital elevation model (DEM), built on **[pytopotoolbox](https://github.com/TopoToolbox/pytopotoolbox)** (TopoToolbox for Python).

The code is a Python port of Paul Morgan’s **[ElevationThresholdValleyWidth](https://github.com/PMonroeMorgan/ElevationThresholdValleyWidth)** workflow, originally written for the MATLAB version of **[TopoToolbox 2](https://github.com/wschwanghart/topotoolbox)**.
...
2. Open **`valley_width_example.ipynb`** and run top to bottom.
3. For your own DEM, copy the notebook sections and adjust **`streamarea`**, **`elevthreshold`** or **`max_valley_width`** settings, **`swath_dx`**, **`minradius`**, and swath length / `'auto'` options as documented in the notebook.
